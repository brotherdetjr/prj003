Feature: api.schedule argument validation
  ...
  Scenario: event name of exactly 63 chars is accepted
    Given emu starts with test script "schedule_name_max/main.lua" and args "--id=DEADBEEF --nowtick=0 --noautotick"
  ...
  Scenario: event name of 64 chars is rejected with a Lua error
    Given emu starts with test script "schedule_name_too_long/main.lua" and args "--id=DEADBEEF --nowtick=0 --noautotick"
    When I spawn a character
    Then the response is ok
    When I get state

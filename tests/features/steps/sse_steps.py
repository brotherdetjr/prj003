...
def step_subscribe_sse(context):
    ...
    def _reader():
        try:
            with requests.get(
                f"http://localhost:{context.port}/events", stream=True, timeout=10
            ) as r:
                ready.set()
                buf = ""
...
@then(
    'I receive a "_on_lua_error" SSE event with fn "{fn}" and error containing "{fragment}"'
)
    ...
    while time.time() < deadline:
        try:
            ...
            if ev.get("event") == "_on_lua_error":
                data = json_module.loads(ev.get("data", "{}"))
                assert data.get("fn") == fn, f"expected fn={fn!r}, got {data}"
            ...
            continue
    assert False, (
        f"timed out waiting for _on_lua_error SSE event (fn={fn!r}, fragment={fragment!r})"
    )

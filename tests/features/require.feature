Feature: Lua require — callbacks in required modules
  ...
  Scenario: deep dotted path via transitive require (main -> effects -> drain)
    ...
    Then the response is ok
    And energy is 10
    And the scheduler has an "effects.drain.on_drain" event at tick 5000

    When I advance to the next event
    Then the response is ok
    And stopped_on_event is true
    And event is "effects.drain.on_drain"

    When I get state
    Then energy is 9
    And the scheduler has an "effects.drain.on_drain" event at tick 10000
